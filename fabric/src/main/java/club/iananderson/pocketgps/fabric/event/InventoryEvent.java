...
public class InventoryEvent {
    ...
    if (PocketGps.curiosLoaded()) {
      ...
      if (trinketInventory.isPresent()) {
        if (trinketInventory.get().isEquipped(item)) {
          ...
          return NBTUtil.getInt(foundTrinketGPS, PocketGps.ENERGY_TAG) > 0;
        }
      }
    }
  ...
  public static void register() {
    ClientTickEvents.END_CLIENT_TICK.register(client -> {
      ...
      if (player != null) {
        boolean hasGpsInv = ItemUtil.hasGps(player, GpsItems.POCKET_GPS);
        boolean hasGpsCurio = findCurio(player, GpsItems.POCKET_GPS);
