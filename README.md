# PocketGPS

This mod adds a "PocketGPS" item and changes minimap mods to only display if it is in the player's
inventory or Curios/Trinkets/Accessories slot. Useful if you want to limit minimap usage in the early game.

Includes support for the following minimaps currently:

- Xaero's Minimap
- FTB Chunks
- JourneyMap

***

## Downloads

#### <ins>Forge/NeoForge/Fabric:</ins>

> [![](https://cf.way2muchnoise.eu/full_pocketgps_downloads.svg?badge_style=flat)](https://www.curseforge.com/minecraft/mc-mods/pocketgps)
> [![Modrinth](https://img.shields.io/badge/dynamic/json?labelColor=black&color=grey&label=&suffix=%20downloads&query=downloads&url=https://api.modrinth.com/v2/project/4VaCQHUg&style=flat-square&logo=data:image/png;base64,)](https://modrinth.com/project/pocketgps)
